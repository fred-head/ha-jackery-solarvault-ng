...
| HTTP (`_find_smartmeter_ip_and_sn`, `_smartmeter_http_poll_loop`, `_create_http_sensors`, `_distribute_http_data`, `_mark_http_sensors_unavailable`) | **Extracted low-level boundary:** `transport/smartmeter_http.py` owns URL construction, the shared-session GET, five-second timeout, status/JSON handling and configured-key numeric validation. Coordinator retains MQTT-cache target discovery, task/cadence, health and all entity effects. | Coordinator `_smartmeter_http_task`, `_http_sm_sensors_created`; failure counter and `last_sm_sn` remain loop locals. Direct transport tests plus loop/lifecycle/isolation tests. | **COMPLETED safe Phase 2 transport portion.** No source, threshold, replacement or entity policy moved. The global creation flag's replacement-meter limitation remains deferred. |
