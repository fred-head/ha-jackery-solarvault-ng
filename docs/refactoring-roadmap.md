...
| 10. Add diagnostics | **P3.1-P3.5 completed:** HA-independent snapshot contract, per-coordinator passive observation state, config-entry HA adapter, adversarial hardening and opt-in structural protocol discovery | Runtime measurements/commands unchanged. The adapter copies fixed owner views and entry-scoped registry aggregates into P3.1 inputs; local built-in snapshots prevent changed-size iteration without adding locks or authority. Protocol discovery is disabled by default, memory-only and observes only bounded structure after host/envelope validation. Broker connectivity remains unknown. | Direct contract, alias/privacy, copy isolation, HTTP/routing observation, loaded/unloaded/partial entry, full-output/pre-redaction canaries, 64-KiB/2,000-child bounds, large registry/discovery, mutation, task, side-effect, multi-entry isolation, discovery bounds/overflow/lifecycle and disabled-mode equivalence tests added. | **P3.1-P3.5 COMPLETED.** Standard-library-only builder/observation/discovery layers remain independent; the HA endpoint stays a read-only adapter. No raw protocol capture, persistence, automatic classification or entity creation was added. |
