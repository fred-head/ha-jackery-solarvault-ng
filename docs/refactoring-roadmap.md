...
| 10. Add diagnostics | Incremental P3.1-P3.5 modules and tests defined in [diagnostics-architecture.md](diagnostics-architecture.md) | Runtime measurements/commands unchanged; passive observation plus a read-only endpoint only. | Contract, full-output canaries, side effects, multiple hosts, failure/recovery, empty cache, malformed data and size limits. | MEDIUM privacy risk. Pure builder, passive observation, HA adapter, adversarial hardening and opt-in protocol discovery remain separately reviewable/revertible. |
