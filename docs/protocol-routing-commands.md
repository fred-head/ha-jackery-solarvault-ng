...
| 23 | Statistics: missing/system/actual host deviceSn means host unless a valid fallback `sn` identifies a devType=1 battery; child devType=1 means battery | Host normalized merge; expansion map retains non-null energy under the canonical `deviceSn`/`sn`; known plugs/plug/cts updated directly, including null | Creates two battery sensors; refreshes batteries or matched plug/CT children; collectors are not searched | Host-SN fix, canonical child-SN matrix, metadata guards, mixed battery discovery/null retention, child search scope |
