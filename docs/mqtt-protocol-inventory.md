...
retain bounded priority. CT/collector calculation selection now follows child
freshness, retains zero aliases and skips entries without measurements. The
historical observations below remain evidence of the earlier implementation.
...
That protocol-hardening PR preserved command envelopes, optimistic timing, list
retention and the then-existing Type-106 policy; the energy follow-up above
supersedes only the relevant power/source behavior.
...
| 2 | Both | Outgoing read-all-settings body=null, eventId=0; incoming status/settings dict (including observed `cmd=106`). | Incoming generic normalization/cache update; nested arrays here replace their cached list rather than using dedicated array merging. The `plug`/`plugs` aliases retain one canonical list. No correlated response validation. | `sensor.py:1535,2251`; `test_mqtt_routing::test_type2_*`, `test_upstream_sync::test_type2_grid_buy_alias_normalized`. |
